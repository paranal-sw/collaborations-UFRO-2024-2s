...
class Core():
    ...
    NECESARIO SAMPLEAR EL DATASET ANTES DE TOKENIZAR.
    """
    ...
    """
        ...
        self.meta_data = load_meta(INSTRUMENT=INSTRUMENT, RANGE=RANGE)
        self.target_data :pd.DataFrame = self.sample_meta()  # es mejor samplear antes de tokenizar.

        #Atributos secundarios
        self.n_errors = len(self.target_data[self.target_data['ERROR']==True])
    ...
    def __str__(self) -> str:
        return f"Core structure: \n Instrument: {self.INSTRUMENT} \n Range: {self.RANGE} \n Clock: {self.clock}"

    
    def load_meta(self, INSTRUMENT: str, RANGE: str, **kwargs) -> None:
        """
    ...
    metadatos correspondientes en el atributo 'meta_data', 'dataset' y 'target_data'. 
    Si la carga de los metadatos falla, se lanza un ValueError.
    ...
    """
        ...
        self.RANGE = RANGE
        try:
            self.dataset = load_dataset(INSTRUMENT=self.INSTRUMENT, RANGE = self.RANGE)
            self.meta_data = load_meta(INSTRUMENT=self.INSTRUMENT, RANGE=self.RANGE)
            self.target_data :pd.DataFrame = self.sample_meta(timer=False)
            if self.time_it and timer: self.clock.stop(func=self.load_meta.__name__)  # end Clock
    ...
    def sample_meta(self, success_ratio: float = 1.0, target_meta= False, **kwargs) -> pd.DataFrame:
        """
    Realiza un muestreo estratificado del DataFrame 'meta_data', extrayendo tanto
    entradas clasificadas como errores ('ERROR' == True) como entradas clasificadas
    como éxitos ('ERROR' == False). El muestreo de éxitos es proporcional al ratio indicado.

    Si 'target_meta' se establece en 'True', se devuelve todo el DataFrame sin realizar el muestreo,
    permitiendo usar todos los datos disponibles.

    Parameters:
    ----------
    success_ratio : float, opcional (default = 1.0)
        Proporción de entradas exitosas ('ERROR' == False) a muestrear en relación con el número de errores ('ERROR' == True).
        Por ejemplo, un valor de '1.0' significa que se toma una cantidad de registros exitosos igual al número de errores.
    
    target_meta : bool, opcional (default = False)
        Si es 'True', no se realiza ningún muestreo y se apunta al DataFrame completo 'meta_data'.

    **kwargs : dict, opcional
        'timer', activa o desactiva la medición de tiempo.

    Returns:
    -------
    pd.DataFrame
        El DataFrame resultante tras el muestreo estratificado de registros exitosos y errores.
    """
        # begin Clock{
        timer = kwargs.get('timer', True)
        if self.time_it and timer: 
        ...
        if target_meta:  # Si target_meta es True apuntará a la misma celda de memoria que meta_data.
            self.target_data = self.meta_data
            return self.target_data
        sample_success = int(len(self.meta_data[self.meta_data['ERROR']==True])*success_ratio)
        meta_errors = self.meta_data[self.meta_data['ERROR']==True]  # filtro
        try:
            meta_success = self.meta_data[self.meta_data['ERROR'] == False].sample(n=sample_success)
        except:
            print("Incompatible success_ratio")
            pass
        self.target_data = pd.concat([meta_success, meta_errors])
        if self.time_it and timer: self.clock.stop(func=self.sample_meta.__name__)  # end Clock
        return self.target_data
    ...
    def target_tokenization(self,**kwargs):
        """  
    ...
    """
        ...
        trace_tokens_list = []
        for k in range(0, len(self.target_data)):  # Recorrer todas las entradas de meta_data
            ...
            gaps = kwargs.get('gaps',False)
            ### } 
            # Tokenizar la traza y agregar los resultados a la lista, guarda la expresión regular utilizada
            self.regExp = regexp
            trace_tokens_list.append(self.trace_tokenization(setting=setting, regexp=regexp, gaps=gaps, timer=False).tolist())
        self.target_data['Trace_tokens'] = trace_tokens_list
        if self.time_it and timer: self.clock.stop(func=self.target_tokenization.__name__)  # end Clock
        pass

    
    def target_vectorization(self,method: str = 'CountVectorizer',**kwargs) -> None:
        """
    ...
    """
        ...
        self.target_data['Classification'] = self.target_data['ERROR'].to_numpy().astype(int)
        ...
        if method == 'CountVectorizer':
            ...
            self.target_data['Word_vector'] = self.target_data['Trace_tokens'].apply(lambda token_list:' '.join([' '.join(tokens) for tokens in token_list]))
            
            # Asignar un vector a cada sucesión.
            vector_series = vectorizer.fit_transform(self.target_data['Word_vector'])
            vector_series = vector_series.toarray().tolist() # transformar a lista
            self.target_data['Word_vector'] = vector_series  # Almacenar la lista de vectores en meta_data
            if timer and self.time_it: self.clock.stop(self.target_vectorization.__name__)  # end Clock
        else:
            pass
    
    
    def transfer_matrix(self, **kwargs) -> tuple:
        """
        Convierte columnas vectoriales a matrices de NumPy.
    ...
    """
        ...
        X = self.target_data['Word_vector']  # Columna con palabras vectorizadas
        X = np.stack(X.values)
        y = self.target_data['Classification'].to_numpy()
        
        if self.time_it and timer: self.clock.stop(func=str(self.transfer_matrix.__name__)) # End Clock
        return X, y
    

    def call_pipeline(self, method: str = 'Multinomial_NB', test_size: float = 0.2,
    ...
    """
        ...
        if method == 'Multinomial_NB':
            X,y = self.transfer_matrix(timer=False)
            ...
            self.pipeline.Multinomial_Naive_Bayes(X_train=X_train,
                                                    ...
                                                    NB_args= NB_kwargs,
                                                    RegExp=self.regExp) # }
            if self.time_it and timer: self.clock.stop(func=str(self.call_pipeline.__name__)+' over '
                                                       +str(self.pipeline.Multinomial_Naive_Bayes.__name__)) # End Clock
        # begin TextBlob {
        elif method == 'TextBlob':
            sample_meta = self.target_data.groupby('Classification',group_keys=False).apply(lambda x: x.sample(frac=test_size))
            ...
            # }
            if self.time_it and timer: self.clock.stop(func=str(self.call_pipeline.__name__)+' over '
                                                       +str(self.pipeline.TextBlob_Sentiment_Analysis.__name__)) # End Clock
    ...
    def meta_predict(self,model) -> dict:
        ...
        if isinstance(model,MultinomialNB):
            y_pred = model.predict(np.stack(self.target_data['Word_vector'].values))
            matrix = confusion_matrix(y_true=self.target_data['Classification'].values,
                                      y_pred=y_pred)
            accuracy = accuracy_score(y_true=self.target_data['Classification'].values,
                                      y_pred=y_pred)
            precision = precision_score(y_true=self.target_data['Classification'].values,
                                        y_pred=y_pred)
            recall = recall_score(y_true=self.target_data['Classification'].values,
                                  y_pred=y_pred)
            summary = {
...
class Custom_Pipeline():
    ...
    Args:
        ...
        RegExp (str)q     : Expresión regular utilizada en el análisis para seguimiento o documentación.
    ...
    def TextBlob_Sentiment_Analysis(self,treshold: float,  meta_data: pd.DataFrame, regExp: str):
        """
...
class TextBlob_Classifier:
    ...
    def __repr__(self) -> str:
        return f"'TextBlob_Classifier'(treshold={self.treshold})"
    ...
    """
        ...
        if conteo_positivo != 0 and conteo_negativo/conteo_positivo < self.treshold:
            return 0
...
class Clock():
    ...
    Attributes:
        ...
        history (list): Historial de métodos ejecutados.
    """
    def __init__(self) -> None:
        """
    ...
    def stop(self, func: str):
        ...
        Almacena en el atributo 'history' el tiempo de ejecución con intención de seguimiento.
        Args:
        ...
        if self.tic is None:
            return None
        execution_time = self.toc - self.tic
        print("Method: ",func,"  Execution time: ",execution_time," Seconds")
        self.history.append({'Function/method' : func,
                             'Performance time' : execution_time})
        self.tic = None
        self.toc = None
        pass
